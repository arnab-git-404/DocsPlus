...
export async function POST(
  ...
  try {
    ...
    console.log('📧 Sending offer letter email to:', offerLetter.candidateEmail);

    const pdfFilename = `OfferLetter_${offerLetter.offerNumber}.pdf`;

    const pdfDocument = React.createElement(OfferLetterDocument, {
      data: offerLetter.toObject(),
    });

    await sendEmail({
      to: offerLetter.candidateEmail,
      subject: `Job Offer - ${offerLetter.position}`,
    ...
    return NextResponse.json({
      ...
      sentTo: offerLetter.candidateEmail,
    });
  } catch (error: any) {
